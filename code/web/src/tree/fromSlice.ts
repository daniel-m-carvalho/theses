...
export interface SliceTreeOptions {
  /**
   * Print each collapsed clade's leaf count beside it.
   *
   * Off by default. A wedge has no identifier of its own — it is a group of
   * sequence types, not one — so its count is the only thing there is to say
   * about it, and saying it for all of them at once is a lot: this slice has
   * 31 clades hiding 13,349 down to 2. Whether that is worth the clutter is
   * the reader's call, so it is a switch rather than a threshold guessed here.
   */
  labelClades?: boolean;
}
...
function nameFor(
  ...
  if (isWedge && labelClades) return `${standsFor.toLocaleString()} leaves`;
  return "";
}

export function treeFromSlice(
  slice: TreeSlice,
  ...
  for (let k = 0; k < count; k += 1) {
    const node: NewickNode = {
      name: nameFor(label[k], truncated[k], true_leaf_count[k], options.labelClades ?? false),
