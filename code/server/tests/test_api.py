...
def test_pairs_are_offered_on_measured_label_overlap(client):
    ...
    pairs = {p["id"]: p for p in client.get("/api/datasets").json()["pairs"]}
    assert set(pairs) == {
...
def test_openapi_document_is_generated(client):
    ...
    assert "/api/datasets" in doc["paths"]
...
def test_a_pair_lists_its_computed_metrics(client, real_store):
    ...
    before = {p["id"]: p for p in client.get("/api/datasets").json()["pairs"]}
    assert before[pair_id]["metrics"] == []

    assert compute_pairs(store_dir=real_store, metrics=["rf"], only=pair_id) == 0
    after = {p["id"]: p for p in client.get("/api/datasets").json()["pairs"]}
...
def test_tree_detail(client):
    body = client.get("/api/trees/vibrio-upgma").json()
...
def test_tree_detail_records_canonicalisation(client):
    ...
    assert client.get("/api/trees/vibrio-nj").json()["suppressed_unary"] == 1


def test_unknown_tree_is_404(client):
    assert client.get("/api/trees/nope").status_code == 404


def test_slice_respects_the_budget_and_loses_nothing(client):
    body = client.get(
        "/api/trees/vibrio-upgma/slice", params={"root": 0, "budget": 50}
    ).json()
...
def test_slice_arrays_are_all_the_same_length(client):
    n = client.get("/api/trees/vibrio-upgma/slice", params={"budget": 100}).json()["nodes"]
    lengths = {k: len(v) for k, v in n.items()}
    assert len(set(lengths.values())) == 1, lengths


def test_slice_of_a_subtree_is_rooted_there(client):
    top = client.get("/api/trees/vibrio-upgma/slice", params={"budget": 20}).json()
    wedge = next(
        i for i, cut in zip(top["nodes"]["id"], top["nodes"]["truncated"]) if cut
    )
    sub = client.get(
        "/api/trees/vibrio-upgma/slice", params={"root": wedge, "budget": 30}
    ).json()
...
def test_slice_rejects_a_root_outside_the_tree(client):
    r = client.get("/api/trees/vibrio-upgma/slice", params={"root": 99_999_999})
    assert r.status_code == 404
    assert r.json()["code"] == "node_out_of_range"


def test_slice_rejects_a_nonsense_budget(client):
    assert client.get("/api/trees/vibrio-upgma/slice", params={"budget": 0}).status_code == 422
    assert client.get("/api/trees/vibrio-upgma/slice", params={"budget": -5}).status_code == 422
    assert client.get(
        "/api/trees/vibrio-upgma/slice", params={"budget": 10_000_000}
    ).status_code == 422
...
def test_slice_is_fast_enough_to_feel_instant(client):
    ...
    client.get("/api/trees/vibrio-upgma/slice", params={"budget": 500})  # warm the maps
    started = time.perf_counter()
    for _ in range(5):
        client.get("/api/trees/vibrio-upgma/slice", params={"budget": 500})
